...
public class AudioProcessor : MonoBehaviour
{
    ...
    /// Dance Code ///
    ////////////////
    private bool neckMoved = false;
    private bool backMoved = false;
    ...
    private float secondsPerBeat = 60.0f / 238; //(60 s/min) / (117 BPM)
    private float secondsSinceSceneLoad = 0.0f;
    private float secondsToWaitBeforeStart = 0.0f;

    // Update is called once per frame
    void Update()
    {
        secondsSinceSceneLoad += Time.deltaTime;

        if(secondsSinceSceneLoad >= secondsToWaitBeforeStart) {
            timeSinceLastBeat += Time.deltaTime;
            if (timeSinceLastBeat >= secondsPerBeat) {
                timeSinceLastBeat = 0.0f;

                Debug.Log(dancer.transform.Find("Hips/Spine/Spine1/Spine2/Neck") != null);
                Transform neck = dancer.transform.Find("Hips/Spine/Spine1/Spine2/Neck");
                Transform back = dancer.transform.Find("Hips/Spine");
                Transform leftLeg = dancer.transform.Find("Hips/LeftUpLeg");


                /*if (!neckMoved) {
                    Vector3 neckEulerAngles = neck.transform.localEulerAngles; //rotation.eulerAngles;
                    neckEulerAngles.x = 19f;
                    neckEulerAngles.y = 371f;
                    neckEulerAngles.z = -33f; //19,371,-33 from 13,355,12                    
                    neck.transform.localRotation = Quaternion.Euler(neckEulerAngles.x, neckEulerAngles.y, neckEulerAngles.z);
                    neckMoved = true;
                } else {
                    neck.transform.localRotation = Quaternion.Euler(13f,355f,12f);
                    neckMoved = false;
                }*/

                if (!backMoved) { //47.9, 395, -12
                    back.transform.localRotation = Quaternion.Euler(21f, 353f, 10.4f);
                    leftLeg.transform.localRotation = Quaternion.Euler(315f, 320f, 187f); //315, 320, 187 -> 253.3,309.9,186.7
                    backMoved = true;
                }
                else { //21,353,10.4
                    Vector3 eulerAngles = back.transform.localEulerAngles; //rotation.eulerAngles;
                    leftLeg.transform.localRotation = Quaternion.Euler(253f, 309.9f, 186.7f);
                    eulerAngles.x = 47.9f;
                    eulerAngles.y = 395f;
                    eulerAngles.z = -12f; //19,371,-33 from 13,355,12                    
                    back.transform.localRotation = Quaternion.Euler(eulerAngles.x, eulerAngles.y, eulerAngles.z);
                    backMoved = false;
                }
        ...
        Vector3 tv = GameObject.Find("PaneGlassBackground").transform.rotation.eulerAngles;
        tv.y += Time.deltaTime * 1.5f;
        GameObject.Find("PaneGlassBackground").transform.rotation = Quaternion.Euler(tv.x, tv.y, tv.z);

        //Vector3 tv = GameObject.Find("PaneGlassBackground").transform.rotation.eulerAngles;
        //tv.y += Time.deltaTime * 1.5f;
        mat = GameObject.Find("DistortionBackground").GetComponent<Renderer>().material;
        //Texture tt = mat.GetTexture("Normalmap");
        mat.SetTextureOffset("Normalmap", new Vector2(.25f, .15f));

        //mat.mainTextureOffset.x = offset;
        //texOffset += Time.deltaTime * 1.5f;
        //mat.SetTextureOffset("bump", new Vector2(texOffset, 0));
        //mat.mainTextureOffset.x = offset;
        ...
        if (audioSource.isPlaying)
        {
            ...
            if (smaxix == now)
            {
                ...
                if (sinceLast > tempopd / 4)
                {
                    ...
                    //Debug.Log(Camera.main.GetComponent<RippleEffect>()); //.Ripple();
    ...
    public void changeCameraColor()
    {
        ...
        GameObject.Find("ColorLight1").GetComponent<Light>().color = color;
        GameObject.Find("Spotlight").GetComponent<Light>().color = color2;
